...
namespace KyungsinLPR
{
    public static class NoDriving
    {
        ...
        public static void Save()
        {
            ...
            Util.Function.IniWriteValue("NODRIVE", "MENT1", Ment1);
        ...
        public static bool Check(string CarNo)
        {
            if (Use)
            {
                if (CarNo.Length > 5 && CarNo != "No_Detection")
                {
                    int LastNo = -1;
                    int.TryParse(CarNo.Substring(CarNo.Length - 1, 1), out LastNo);
                    int mod = DateTime.Now.Day % (int)Option;

                    //5부제, 10부제
                    if((int)Option != 2) {
                        if(mod == LastNo % (int)Option) {
                            return true;
                        } else {
                            return false;
                        }
